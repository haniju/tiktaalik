...
- **Tool persistence on drawing open**: all tool settings (colors, widths, opacities, smoothing, canvas mode) are persisted in localStorage and restored when the app loads. **Exception: the text tool is never restored from persistence.** If the last active tool was `text`, it resets to `pen` on next load. Rationale: the text tool creates a new TextBox on any canvas tap (idle state), so restoring it means the user unintentionally creates a TB the moment they touch the canvas after opening a drawing. This is a deliberate UX decision — the text tool requires an explicit user action (tap on Drawingbar) to activate.
